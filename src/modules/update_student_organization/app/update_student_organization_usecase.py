...
class UpdateStudentOrganizationUsecase:
    ...
    def __call__(self, stu_org_id: str, name: Optional[str] = None, description: Optional[str] = None, creation_date: Optional[str] = None, logo: Optional[str] = None, instagram: Optional[str] = None):
        
        if not self.repo.get_stu_org(stu_org_id):
            raise NoItemsFound("Student Organization")
        
        return self.repo.update_stu_org(stu_org_id, name, description, creation_date, logo, instagram)